...
base = np.array(converted_conditions["付与無し"])
...
for name in [
    ...
    scores = np.array(converted_conditions[name])
    ...
    r, p = pearsonr(base, scores)

    kappa = cohen_kappa_score(base, scores)

    accuracy = np.mean(base == scores)
