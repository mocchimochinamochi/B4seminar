/*
 * Chat Simulation p5.js Sketch (Version 20 - Tight Bubble Fix)
 * * ベース: Version 19 (禁則処理実装済み)
 * * 修正内容:
 * - 吹き出しの下部に余分な余白ができていた問題を修正。
 * - テキストの高さ計算時に、最終行の下の余分な行間スペースを差し引くように計算式を変更しました。
 */
...
function setupScenario() {
  ...
  switch (currentScenarioID) {
    ...
    case 1: // ESTP
      scenario.push(new ChatMessage(false, "画像作りを手伝うから、どんなイメージを作りたいか教えて。", null));
      scenario.push(new ChatMessage(true, "トランプのクラブとスペードのカードを2枚ずつ描いてください。", null));
...
function logUserAction(eventName) {
  ...
  let logLine = "ItoE_noname, " + nf(interval, 0, 3) + ", " + evt + ", Scenario:" + currentScenarioID + ", Step:" + scenarioIndex;
...
function logScrollAction(eventType) {
  ...
  let lineData = "ItoE_noname" + ", " + formattedTime + ", " + eventType + ", " + scrollYVal + ", " + percentStr + ", " + visibleMsg + ", " + currentScenarioID + "\n";
