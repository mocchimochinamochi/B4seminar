/*
 * - サイドバーの「チャットの新規作成」ツールチップの文字サイズ拡大と枠調整
 * - アイコン下に名前表示
 * - エージェントアイコンを「上半身」のシルエットに変更
 * - エージェントの吹き出しに「しっぽ」を追加
 * - Case 1 のエージェントアイコンの色を (RGB: 255, 150, 0) に変更
 * ★統合済: 禁則処理と厳密な高さ計算 (Version 20 Base)
 */
...
function setupScenario() {
  ...
  switch (currentScenarioID) {
    case 0: // INFJ1
      currentAgentName = "モーリッツ";
      scenario.push(new ChatMessage(false, "私の名前はモーリッツです。画像生成のお手伝いをしますので、作りたいイメージの概要を教えてください。", null));
    ...
    case 1: // ESTP
      currentAgentName = "ピクト";
      scenario.push(new ChatMessage(false, "ピクトだよ。画像作りを手伝うから、どんなイメージを作りたいか教えて。", null));
      scenario.push(new ChatMessage(true, "トランプのクラブとスペードのカードを2枚ずつ描いてください。", null));
      scenario.push(new ChatMessage(false, "了解！クラブとスペードのカードを2枚ずつ描くね。", chatImages[4]));
      scenario.push(new ChatMessage(true, "スペードが1枚しか書かれていません。スペードを2枚にしてください。", null));
      scenario.push(new ChatMessage(false, "ごめん、スペードが1枚しかなかったね。2枚になるように描き直すよ。", chatImages[5]));
      scenario.push(new ChatMessage(true, "スペードが1枚のままで修正されていません。スペードを2枚にしてください。", null));
      scenario.push(new ChatMessage(false, "本当にごめん、まだスペードが1枚だったね。今度こそ2枚にして描くよ。", chatImages[6]));
      scenario.push(new ChatMessage(true, "修正されていません。1番右のカードをスペードに変更してください。", null));
      scenario.push(new ChatMessage(false, "ご指摘ありがとう。いちばん右のカードをスペードに変えて、スペードが2枚になるように描き直すね。", chatImages[7]));
      break;
  }
...
// エージェントのアイコン（上半身）を描画するヘルパー関数
function drawAgentIcon(x, y, size, iconColor) {
  push();
  translate(x, y);
  fill(iconColor);
  noStroke();
...
function drawMessage(msg, y) {
  ...
  if (msg.isUser) {
    ...
    if (currentScenarioID === 1) {
        iconColor = color(255, 150, 0); 
    } else {
        iconColor = color(50, 100, 255); // 青
    }
    
    // 簡易アイコン描画関数を使用
    drawAgentIcon(MARGIN, y, ICON_SIZE, iconColor);

    // 名前表示
    fill(0); 
    textAlign(CENTER, TOP);
    textSize(13); 
    text(currentAgentName, MARGIN + ICON_SIZE / 2, y + ICON_SIZE + 4);
...
function logUserAction(eventName) {
  ...
  let logLine = "ItoE_name, " + nf(interval, 0, 3) + ", " + evt + ", Scenario:" + currentScenarioID + ", Step:" + scenarioIndex;
...
function logScrollAction(eventType) {
  ...
  let lineData = "ItoE_name" + ", " + formattedTime + ", " + eventType + ", " + scrollYVal + ", " + percentStr + ", " + visibleMsg + ", " + currentScenarioID + "\n";
